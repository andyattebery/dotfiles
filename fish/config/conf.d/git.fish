# Inspired from: https://github.com/sorin-ionescu/prezto/blob/master/modules/git/alias.zsh

# Log
! set -q _git_log_medium_format && set _git_log_medium_format '%C(bold)Commit:%C(reset) %C(green)%H%C(red)%d%n%C(bold)Author:%C(reset) %C(cyan)%an <%ae>%n%C(bold)Date:%C(reset)   %C(blue)%ai (%ar)%C(reset)%n%+B'
! set _git_log_oneline_format '%C(green)%h%C(reset) %s%C(red)%d%C(reset)%n'
! set -q _git_log_brief_format && set _git_log_brief_format '%C(green)%h%C(reset) %s%n%C(blue)(%ar by %an)%C(red)%d%C(reset)%n'

# Status
! set -q _git_status_ignore_submodules && set _git_status_ignore_submodules 'all'
...
alias gpsc='git push --set-upstream origin "(git-branch-current 2> /dev/null)"'
alias gpsp='git pull origin "(git-branch-current 2> /dev/null)" && git push origin "(git-branch-current 2> /dev/null)"'
